       ...
       // private Mutex mutex = new Mutex();
        ...
        private void device_OnPacketArrival1(object sender, CaptureEventArgs packet)
        {
                ...
                lock (locker)
                {
                    ...
                    {
                        var IPv4Packet = (PacketDotNet.IPv4Packet)ethernetPacket.Extract(typeof(PacketDotNet.IPv4Packet));

                        //   Console.WriteLine("ja som " + protokol + "\n");
                        if (IPv4Packet != null && checkFilter(portNumber, "In", ethernetPacket.SourceHwAddress.ToString()
                            , ethernetPacket.DestinationHwAddress.ToString(), IPv4Packet.SourceAddress.ToString(),
                            IPv4Packet.DestinationAddress.ToString(), protokol) == 1)
                        {
                            return;
                        }
                    }
                 ...
                 //       + "---------");
                    ...
                    else
                    {
                        ...
                        //   Console.WriteLine("ja som " + protokol + "\n");
                        if (IPv4Packet != null && checkFilter(destPort, "Out", ethernetPacket.SourceHwAddress.ToString()
                            , ethernetPacket.DestinationHwAddress.ToString(), IPv4Packet.SourceAddress.ToString(),
                            IPv4Packet.DestinationAddress.ToString(), protokol) == 1)
                        {
                            return;
            ...
            if (packet != null)
            {
                ...
                if (packet.Type.ToString().Equals("IpV4"))
                {
                  ...
                  //  Console.WriteLine("Som v IPv4");
                    ...
                    var IPv4Packet = (PacketDotNet.IPv4Packet)packet.Extract(typeof(PacketDotNet.IPv4Packet));

                    //System.Diagnostics.Trace.WriteLine("{0}", IPv4Packet.Protocol.ToString());

                    if (IPv4Packet.Protocol.ToString().Equals("TCP"))
                    {
                        // System.Diagnostics.Trace.WriteLine("{0}", packet.Type.ToString());
                        //    Console.WriteLine("Som v TCP");
                        var TcpPacket = (PacketDotNet.TcpPacket)IPv4Packet.Extract(typeof(PacketDotNet.TcpPacket));
                    ...
                    else if (IPv4Packet.Protocol.ToString().Equals("UDP"))
                    {
                        //  Console.WriteLine("Som v UDP");

                        return "UDP";
                    }
                    else if (IPv4Packet.Protocol.ToString().Equals("ICMP"))
                    {
                        var icmpPacket = (PacketDotNet.ICMPv4Packet)IPv4Packet.Extract(typeof(PacketDotNet.ICMPv4Packet));
